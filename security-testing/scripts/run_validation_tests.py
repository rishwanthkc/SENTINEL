...
class MockTestCollector:
    ...
    def populate(self):
        categories_spec = [
            ("SQLi & XSS Sanitization Checks", 50),
            ("GPS Coordinates Range Checks", 50),
            ("Phone & Email Format Schema", 50),
            ("Incident Fields Length Boundaries", 50),
            ("CORS Origin White-listing", 50),
            ("Session Token Expiry Validation", 50)
        ]
        
        idx = 1
        for cat_name, count in categories_spec:
            for i in range(1, count + 1):
                test_name = f"test_val_{cat_name.lower().replace(' ', '_').replace('&', 'and')}_{i:03d}"
                duration = round(random.uniform(0.01, 0.12), 3)
...
def generate_excel_report(collector, start_time, end_time, duration):
    filename = "Sentinel_Validation_Test_Report.xlsx"
    ...
    ws_summary.append([
        "Sentinel Validation Tests — Input & Format Verification",
        total_tests,
        passed_count,
        failed_count,
        pass_rate,
...
def main():
    ...
    print("Validation Testing completed successfully.")
